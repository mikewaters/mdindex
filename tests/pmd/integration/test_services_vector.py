...
class TestServiceEmbedCollection:
    ...
    async def test_embed_collection_creates_embeddings(
        ...
        async with await create_application(config) as app:
            ...
            docs = app.document_repo.list_by_collection(collection.id)
            for doc in docs:
                assert app.embedding_repo.has_embeddings(doc.hash)
    ...
    async def test_embed_collection_skips_existing(
        ...
        async with await create_application(config) as app:
            ...
            # First embedding
            result1 = await app.indexing.embed_collection("test")
            assert result1.embedded == 1

            # Second embedding should skip
            result2 = await app.indexing.embed_collection("test")
    ...
    async def test_embed_collection_force_reembeds(
        ...
        async with await create_application(config) as app:
            ...
            # Force re-embed
            result = await app.indexing.embed_collection("test", force=True)
...
class TestServiceVectorSearch:
    ...
    async def test_vector_search_respects_collection_filter(
        ...
        async with await create_application(config) as app:
            ...
            await app.indexing.embed_collection("coll1")
            await app.indexing.embed_collection("coll2")
...
class TestServiceHybridSearch:
    ...
    async def test_hybrid_search_fts_only_fallback(
        ...
        async with await create_application(config) as app:
            ...
            results = await app.search.hybrid_search("searchable", limit=5)
...
class TestServiceCorpusSearch:
    ...
    async def test_corpus_vector_search(self, mlx_config: MLXConfig, tmp_path: Path):
        ...
        async with await create_application(config) as app:
            ...
            app.source_collection_repo.create("corpus", str(corpus_path), "**/*.md")
            source = _filesystem_source_for_name(app, "corpus")
            await app.indexing.index_collection("corpus", source)

            # Only embed first 20 docs for speed
            result = await app.indexing.embed_collection("corpus")
            ...
            for query, expected_substr in search_cases:
                results = await app.search.vector_search(query, limit=10)
