...
class MetadataProfileRegistry:
    ...
    Example:
        ...
        registry.register(GenericProfile(), priority=-1)  # Fallback
    ...
    def register(
        ...
        *,
        ...
        Args:
            profile: Profile instance implementing MetadataProfile.
            path_patterns: Regex patterns for path-based matching.
            detectors: Functions for content-based detection.
            priority: Higher priority profiles are checked first.
            override: If True, allow overriding an existing registration.

        Raises:
            ValueError: If profile name is already registered and override=False.
        """
        name = profile.name
    ...
    def unregister(self, name: str) -> bool:
        ...
        Returns:
            True if the profile was registered and removed, False if not found.
        """
    ...
    def get(self, name: str) -> "MetadataProfile | None":
        ...
        Returns:
            MetadataProfile instance if found, None otherwise.
        """
    ...
    def detect(self, content: str, path: str) -> "MetadataProfile | None":
        ...
        Checks registrations in priority order:
        1. First, checks path patterns
        2. Then, runs detector functions
        3. Returns the first matching profile

        Args:
            content: Document content for content-based detection.
            path: Document path for path-based matching.

        Returns:
            Matching MetadataProfile or None if no match.
        """
    ...
    def detect_or_default(
        ...
        Args:
            ...
            path: Document path.
            default_name: Name of the default profile if no match.

        Returns:
            Detected profile or the default profile.

        Raises:
            ValueError: If the default profile is not registered.
        """
    ...
    def is_registered(self, name: str) -> bool:
        ...
        Returns:
            True if the profile is registered.
        """
...
def get_default_profile_registry() -> MetadataProfileRegistry:
    ...
    The default registry is lazily initialized with built-in profiles.

    Returns:
        The default MetadataProfileRegistry instance.
    """
...
def _register_builtin_profiles(registry: MetadataProfileRegistry) -> None:
    ...
    registry.register(
        ...
        detectors=[_detect_obsidian_content],
        priority=10,
    )
    ...
    registry.register(
        ...
        detectors=[_detect_drafts_content],
        priority=10,
    )
...
def _detect_obsidian_content(content: str, path: str) -> bool:
    ...
    Looks for Obsidian-specific patterns:
    - [[wikilinks]]
    - ![[embedded content]]
    - %%comments%%
    - Nested tags in frontmatter

    Args:
        content: Document content.
        path: Document path (unused).

    Returns:
        True if content appears to be from Obsidian.
    """
...
def _detect_drafts_content(content: str, path: str) -> bool:
    ...
    Looks for Drafts-specific patterns:
    - UUID in frontmatter
    - Drafts-style metadata fields
    ...
    Returns:
        True if content appears to be from Drafts.
    """
